---
title: "KSPZV1 DGE Δ (postvax/baseline) - Placebo limma voom - revision"
author: "Tuan M. Tran"
date: "08/092022"
...
This analysis is for Placebo group.
...
# save(design, fit, fit2, DeltaProtected, DeltaNotProtected, DeltaDelta, file = "/Volumes/GoogleDrive/My Drive/Manuscripts/KSPZV1 Manuscript/Med Revised Submission 2022/Revision R Objects/DeltaDelta Model Placebo only Batch Sex Site CSPAb MALdVaxTotal adj.RData")
```


### Examine top 15 DEGs in Placebo group
...
all_bound %>%
  ...
  writexl::write_xlsx(., path = paste0(resdir, "Table S3 Delta DEG Tables for Placebo KSPZV1 REVISED.xlsx"))

#combine with Table S3 delta high-dose PfSPZ

foo1 <- readxl::read_excel("/Volumes/GoogleDrive/My Drive/Manuscripts/KSPZV1 Manuscript/Med Revised Submission 2022/Supplementary Tables/Table S3 Delta DEG Tables for high-dose PfSPZ KSPZV1 REVISED.xlsx")

writexl::write_xlsx(list(high_dose = foo1, placebo = all_bound),
                    path = "/Volumes/GoogleDrive/My Drive/Manuscripts/KSPZV1 Manuscript/Med Revised Submission 2022/Supplementary Tables/Table S3 Delta DEG Tables for HighDose KSPZV1 and Placebo REVISED.xlsx")
```

### See if genes in Fig 4H Revised are differentially expressed in Placebo

```{r rev1 check}
mygenes <- c("RP11-440L14.1", "MED28", "FSTL4", "KAT7", "TMEM220-AS1")

all_bound %>%
  filter(hgnc_symbol %in% mygenes) %>%
  filter(comparison == "ΔP vs ΔNP") %>%
  select(dosegroup, timepoint, comparison, hgnc_symbol, logFC, P.Value, adj.P.Val) %>%
  writexl::write_xlsx(., path = "/Volumes/GoogleDrive/My Drive/Manuscripts/KSPZV1 Manuscript/Med Revised Submission 2022/Supplementary Tables/Table R1 delta highdose and placebo.xlsx")
```
...
allGSEA %>%
  writexl::write_xlsx(., path = paste0(resdir, "Table SX Delta GSEA Tables for Placebo KSPZV1 REVISED.xlsx"))
```
