...
names(df.list) <- gsub(" UpReg LFC 263 pval25 with logCSP.xls", "", myFiles)
...
myUpRegPlot <- UpRegPvS_foo %>%
  ...
  ggrepel::geom_label_repel(data = UpRegPvS_label_df, force = 15, force_pull = 2, size = 3.4, max.overlaps = 25,
                            label.size = NA,
                            label.padding = 0.01,
                            box.padding = 0.01,
...
cairo_pdf(paste0(figdir, "Fig 2E IPA UpReg 4 doses 1x4 revised logCSP.pdf"), height = 6.55, width = 24)
myUpRegPlot
dev.off()
```