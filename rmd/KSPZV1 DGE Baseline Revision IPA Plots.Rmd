...
myFiles <- list.files(path = datadir, pattern='*263 pval25.xls')
df.list <- lapply(paste0(datadir, myFiles), read_excel, skip = 1)
names(df.list) <- gsub(" UpReg LFC 263 pval25.xls", "", myFiles)
...
UpRegPvS_label_df <-  UpRegPvS_foo %>%
  dplyr::filter(label %in% foo$label | #limit labels due to space limitations
                  (`Activation z-score` < -2 & `B-H corrected p-value` < significance.cutoff & `Dose group` == "Placebo")|
                  (`Activation z-score` > 2.2 & `B-H corrected p-value` < significance.cutoff & `Dose group` == "Placebo")|
                  (`Activation z-score` < -2 & `B-H corrected p-value` < significance.cutoff & `Dose group` == "4.5x105")|
                  (`Activation z-score` > 2 & `B-H corrected p-value` < significance.cutoff & `Dose group` == "4.5x105")|
                  (`Activation z-score` < -2 & `B-H corrected p-value` < significance.cutoff & `Dose group` == "4.5x105")|
                  (`Activation z-score` > 2 & `B-H corrected p-value` < significance.cutoff & `Dose group` == "9.0x105")|
                  (`Activation z-score` < -2.2 & `B-H corrected p-value` < significance.cutoff & `Dose group` == "9.0x105")|
...
myUpRegPlot <- UpRegPvS_foo %>%
  ...
  ggrepel::geom_label_repel(data = UpRegPvS_label_df, force = 18, force_pull = 1, size = 3.5, max.overlaps = 20,
                            label.size = NA,
                            label.padding = 0.01,
                            box.padding = 0.01,
...
cairo_pdf(paste0(figdir, "Fig 2E IPA UpReg 4 doses 1x4 revised.pdf"), height = 6.25, width = 24.25)
myUpRegPlot
dev.off()
```