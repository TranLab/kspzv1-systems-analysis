...
knitr::kable(gois_ms)
```
### Determine if 6 genes are present in Placebo

```{r determine presence in placebo, warning=FALSE, message=FALSE}
DeltaProtected$Placebo %>%
  filter(Geneid %in%gois_ms$GeneSymbol) %>%
  dplyr::select(GeneSymbol, logFC, logCPM, F, PValue, FDR)

DeltaNotProtected$Placebo %>%
  filter(Geneid %in%gois_ms$GeneSymbol) %>%
  dplyr::select(GeneSymbol, logFC, logCPM, F, PValue, FDR)
