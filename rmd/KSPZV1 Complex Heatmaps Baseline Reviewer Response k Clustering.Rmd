...
```{r optimal clusters Silhouette, echo=FALSE, message=FALSE, warning=FALSE}
# Silhouette method
# fviz_nbclust(t(exprs(x)), cluster::pam, method = "silhouette")+
#   labs(subtitle = "Silhouette method")
```

```{r optimal clusters Gap statistic, echo=FALSE, message=FALSE, warning=FALSE}
# Gap statistic
# nboot = 50 to keep the function speedy. 
# recommended value: nboot= 500 for your analysis.
# Use verbose = FALSE to hide computing progression.
#set.seed(123)
# fviz_nbclust(t(exprs(x)), cluster::pam, nstart = 25,  method = "gap_stat", nboot = 100)+
#   labs(subtitle = "Gap statistic method")

# myclusGapres <- cluster::clusGap(t(exprs(x)), cluster::pam, method = "firstSEmax", K.max = 8, B = 60)
# title <- "Raw data"
# par(mfrow=c(1,2))
# xy <- t(exprs(x))
# for (i in 1:2) {
#   k <- maxSE(myclusGapres$Tab[, "gap"], myclusGapres$Tab[, "SE.sim"], method="Tibs2001SEmax")
#   fit <- kmeans(xy, k)
#   #
#   # Plot the results.
#   #
...
#   plot(myclusGapres, main=paste("Gap stats,", title))
...
## Determin Optimal number of clusters 
