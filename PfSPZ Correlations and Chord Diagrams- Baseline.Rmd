...
temp <- tempfile(fileext = ".rds")
dl <- drive_download(
  as_id("10G9UVgNmg3M6YZjfWYmbk0UWw9HfMjPL"), path = temp, overwrite = TRUE)
...
## Append the post-vaccination CSP-specific IgG to the baseline parameters of 'alldat' and remove baseline CSP-specific IgG
facs_timezero<-facs %>% filter(grepl("_0", PATID.OG))%>%dplyr::mutate(.,"PATID"=  str_replace(PATID.OG, '_.*',''))
postvax_csp<-facs%>% dplyr::mutate(.,"PATID"=  str_replace(PATID.OG, '_.*',''))%>% filter(grepl("_25", PATID.OG))
postvax_csp<-postvax_csp[,c(1,3, ncol(postvax_csp))]
x<-merge(facs_timezero, postvax_csp, by="PATID")
baseline_facs<-x%>% dplyr::rename("PATID.OG"="PATID.OG.x")%>%dplyr::select(!contains("anti-CSP IgG.x"))%>%dplyr::select(!contains("PATID.OG.y"))
nrow(baseline_facs) ## should be 337
```
...
#Merge
alldata<-pheno_data%>%left_join(., baseline_facs, by="PATID.OG")
```
...
pas_m.df<-t(pas_m.df)  ## see next chunk for delta
...
#Applicable to baseline data only: Columns that have a large percentage of zero values and a few high leverage points tend to show up as significant correlations downstream. To avoid this, remove columns that contain ~>75% zero values. Only certain cytokines fell into this category, and by thresholding, we were able to remove them and prevent 'false' correlations. 

```{r remove columns with >75% zeros,  message=FALSE, warning=FALSE}
# For high-level BTMs
i <- colSums(all_data_btm == 0, na.rm=TRUE) < 173  #173 is 75% of 230 (i.e. nrow). 
all_data_btm<-all_data_btm[i]
ncol(all_data_btm) #47

# For Monaco modules
i <- colSums(all_data_monaco == 0, na.rm=TRUE) < 173  #173 is 75% or 230. 
all_data_monaco<-all_data_monaco[i]
ncol(all_data_monaco) #51
```
...
all_data_btm<-all_data_btm %>%
  ...
  dplyr::rename("CD14+CD16- of monocytes"  = "FACS_CD14+CD16-_of_live_monocytes")%>%
  dplyr::rename("CD14-CD16+ of monocytes"  = "FACS_CD14-CD16+_of_live_monocytes")%>%
  dplyr::rename("Postvax anti-CSP IgG"  = "anti-CSP IgG.y")%>%
  ...
  dplyr::rename("CCL2 MCP-1"  = "CytokineObsConc_CCL2_MCP-1")%>%
  dplyr::rename("CCL4 MIP-1-beta"  = "CytokineObsConc_CCL4_MIP-1_beta")%>%
  dplyr::rename("CXCL10 IP-10"  = "CytokineObsConc_CXCL10_IP-10")%>%
  dplyr::rename("IFN-alpha"  = "CytokineObsConc_IFN-alpha")%>%
  dplyr::rename("IL-1-beta"  = "CytokineObsConc_IL-1_beta")%>%
  dplyr::rename("IL-10"  = "CytokineObsConc_IL-10")%>%
  dplyr::rename("IL-6"  = "CytokineObsConc_IL-6")%>%
  dplyr::rename("IL-8 CXCL8"  = "CytokineObsConc_IL-8_CXCL8")%>%
  dplyr::rename("TNF-alpha"  = "CytokineObsConc_TNF-alpha")
...
all_data_btm<-all_data_btm[,-c(1:4, 6)]
all_data_monaco<-all_data_monaco[,-c(1:4, 6)]
```
...
combo3_btm$Data.Type <- ifelse(grepl("CCL2 MCP-1", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("CCL4 MIP-1-beta", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("CXCL10 IP-10", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("GM CSF", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("IFN-alpha", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("IFN-gamma", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("IFN-beta", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("IL-1-beta", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("IL-10", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("IL-12 p70", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("IL-2", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("IL-4", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("IL-6", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("IL-8 CXCL8", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
combo3_btm$Data.Type <- ifelse(grepl("TNF-alpha", rownames(combo3_btm)), "Cytokine", combo3_btm$Data.Type) 
...
combo3_monaco$Data.Type <- ifelse(grepl("CCL2 MCP-1", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("CCL4 MIP-1-beta", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("CXCL10 IP-10", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("GM CSF", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("IFN-alpha", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("IFN-gamma", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("IFN-beta", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("IL-1-beta", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("IL-10", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("IL-12 p70", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("IL-2", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("IL-4", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("IL-6", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("IL-8 CXCL8", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
combo3_monaco$Data.Type <- ifelse(grepl("TNF-alpha", rownames(combo3_monaco)), "Cytokine", combo3_monaco$Data.Type) 
```
...
combo3_btm<-combo3_btm %>% arrange(factor(Data.Type, levels = c("Outcome", "Antibody","Cellular expression",  "Cytokine", "Gene Expression"))) 
...
cairo_pdf("Timpoint_baseline_FDR_0.02_Hilevel_BTMS.pdf", height = 10, width = 10)
...
#single group
#Data type
for(nm in names(gl1_btm)) {
  r = gl1_btm[[nm]]
  highlight.sector(sector.index = r, track.index = 1, col = group_color1_btm[nm], 
                   text = nm, text.vjust = 0, niceFacing = TRUE, padding = c(0,0,0,0), facing = "bending", cex=1)
}
dev.off()
circos.clear()
# Upload the plot
drive_upload("Timpoint_baseline_FDR_0.02_Hilevel_BTMS.pdf")
```
```{r}
palette(brewer.pal(12, "Set3"))
set.seed(12)
...
## to get CSP-specific IgG, outcome and dose together
combo3_monaco<-combo3_monaco %>% arrange(factor(Data.Type, levels = c("Outcome", "Antibody","Cellular expression",  "Cytokine", "Gene Expression"))) 
...
cairo_pdf("Timpoint_baseline_FDR_0.02_Monaco.pdf", height = 10, width = 10)
circos.par(canvas.xlim = c(-1.8,1.8), canvas.ylim = c(-1.8,1.8), points.overflow.warning=FALSE, 
           track.margin = c(0.01, 0.05), # adjust bottom and top margin
           track.height = 0.05, start.degree = 180)
...
#single group
#Data type
for(nm in names(gl1_monaco)) {
  r = gl1_monaco[[nm]]
  highlight.sector(sector.index = r, track.index = 1, col = group_color1_monaco[nm], 
                   text = nm, text.vjust = 0, niceFacing = TRUE, padding = c(0,0,0,0), facing = "bending", cex=1)
}
dev.off()
circos.clear()
# Upload the plot
drive_upload("Timpoint_baseline_FDR_0.02_Monaco.pdf")
```
