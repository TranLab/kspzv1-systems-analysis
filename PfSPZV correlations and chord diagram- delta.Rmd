---
title: "PfSPZ correlation chord diagrams: Delta (Post-vaccination- baseline)"
...
9. This script is specifically for delta (post-vaccination- baseline) features

Notes: 
- Cytokine levels were not included in  the correlations of delta features
...
temp <- tempfile(fileext = ".rds")
dl <- drive_download(
  as_id("10uFfj5fPGyuUlMkITBH52cVXO1rpRwoo"), path = temp, overwrite = TRUE)
...
# Calculate delta non-transcriptomic features
```{r delta facs data, message=FALSE, warning=FALSE}
baseline<-facs%>%filter(grepl("_0", PATID.OG))
nrow(baseline) ##337

postvax<-facs%>%filter(grepl("_25", PATID.OG))
nrow(postvax) ##337

p<-postvax[,c(2:ncol(postvax))]
b<-baseline[,c(2:ncol(baseline))]

delta_facs<-p-b

rownames(delta_facs)<-postvax$PATID.OG
delta_facs<-delta_facs%>%rownames_to_column(., var="PATID_XX")
delta_facs<-delta_facs%>% dplyr::mutate(.,"PATID.OG"=  str_replace(PATID_XX, '_.*',''))
delta_facs<-delta_facs[, -c(1,2)]
delta_facs <- delta_facs %>%
...
#Merge
alldata<-pheno_data%>%left_join(., delta_facs, by="PATID.OG")
```
...
exprs(eset) <- exprs(eset) %>%
  t(.) %>%
  scale(., center = TRUE, scale = TRUE) %>% #generating z scores using scale function
  t(.)
...
rownames(expr_data)<-(expr_data[,231])
expr_data<-expr_data[,-231]
...
all_data_btm<-all_data_btm[,-c(1:4)]
all_data_monaco<-all_data_monaco[,-c(1:4)]
```
...
cairo_pdf("Timpoint_delta_FDR_0.1_Hilevel_BTMS.pdf", height = 10, width = 10)
...
circos.clear()
# Upload the plot
drive_upload("Timpoint_delta_FDR_0.1_Hilevel_BTMS.pdf")
```
...
cairo_pdf("Timpoint_delta_FDR_0.2_Monaco.pdf", height = 10, width = 10)
circos.par(canvas.xlim = c(-1.8,1.8), canvas.ylim = c(-1.8,1.8), points.overflow.warning=FALSE, 
           track.margin = c(0.01, 0.05), # adjust bottom and top margin
           track.height = 0.05, start.degree = 180)
...
highlight.sector(rownames(combo3_monaco)[which(combo3_monaco$Data.Type == "Outcome")],
                 track.index = get.all.track.index(), col = "grey",
                 text = "Outcome", cex = 0.8, text.col = "black", niceFacing = TRUE)
dev.off()
circos.clear()
# Upload the plot
drive_upload("Timpoint_delta_FDR_0.2_Monaco.pdf")
```